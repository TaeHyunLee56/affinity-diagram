...
document.getElementById('fileInput').addEventListener('change', function(event) {
    ...
    reader.onload = function(e) {
        ...
        // 모든 행과 열을 순회하며 각 셀의 내용을 노트로 추가
        jsonData.forEach((row, rowIndex) => {
            row.forEach((noteText, colIndex) => {
                // 빈 셀이거나 undefined인 경우 건너뛰기
                if (noteText === undefined || noteText === null || noteText === '') return;
...
function updateApiKey() {
    ...
    const inputField = document.querySelector('.apikey-input');
    if (newApiKey) {
        apiKey = newApiKey;
